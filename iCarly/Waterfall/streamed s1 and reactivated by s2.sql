...
select
      ...
      case
        when expiration_dt is NOT NULL or expiration_dt >= '2021-06-17' -- '2021-09-01'
        then 'Inactive' 
        else 'Active'
      end active_status,
      activation_dt,
  ...
  (
    ...
    (
      ...
      from
        `i-dss-ent-data.dw_vw.aa_video_detail_reporting_day`
      where
        day_dt between '2022-03-01' and '2022-05-05'
        and reporting_series_nm like '%iCarly%'
        and reporting_content_type_cd = 'FEP'
        --and video_season_nbr = '1'
        and v69_registration_id_nbr is not null
...
distinct cbs_reg_user_id_cd
 from
    ...
    where
      cbs_reg_user_id_cd is not null
      and paid_start_dt is not null  -- winback
      and trial_start_dt is null  -- winback
      --and expiration_dt<paid_start_dt -- winback
      and cbs_reg_user_id_cd not in (select regid from complimentory_users)
      and src_system_id = 115
      and paid_start_dt >= '2022-03-01'
      and cbs_reg_user_id_cd in (select * from inactive_subs))
...
count (distinct v69_registration_id_nbr)
      ...
      --  and video_season_nbr = '1'
        and v69_registration_id_nbr is not null
        and v69_registration_id_nbr in (select * from resub)
